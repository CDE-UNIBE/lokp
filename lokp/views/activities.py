...
from lokp.config.customization import get_customized_template_path, \
    get_mandatory_keys, get_spatial_accuracy_map
...
from lokp.utils.views import get_output_format, get_page_parameters, \
    get_bbox_parameters, get_status_parameter, handle_query_string, \
    validate_uuid, shorten_uuid
...
activity_protocol = ActivityProtocol(DBSession)
...
class ActivityView(BaseView):
    ...
    def review(self):
        ...
        if review_decision == 1 and len(activity.tag_groups) > 0:
            ...
            activity_keys = DBSession.query(A_Key.key). \
                join(A_Tag). \
