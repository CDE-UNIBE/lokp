...
class StakeholderProtocol(Protocol):
    ...
    def read_one_history(
            ...
            translate=True):
        """
        Read all visible versions of a :term:`Stakeholder`. This
        function handles the query, creates and returns the Feature.
        ...
        Returns:
            ...
            query and the :term:`Stakeholder` Features in JSON compatible
            format.
    ...
    def get_relevant_query_one(self, uid, version=None, public_query=False):
        ...
        :term:`Stakeholder`. Usually, a single version (the latest
        visible by default or the one indicated) is returned. However,
        this function can also be used to return all visible version of
        a single :term:`Stakeholder`.
        ...
        Args:
            ...
            * ``int``: An optional version of the :term:`Stakeholder` to
        ...
        relevant_query = relevant_query.\
            outerjoin(order_query, order_query.c.id == Stakeholder.id)
        relevant_query = relevant_query.order_by(desc(Stakeholder.version))
        relevant_query = relevant_query.\
            group_by(
    ...
    def query_many_only_id(self, relevant_query):
        ...
        query = self.Session.query(
            ...
            join(relevant_query, relevant_query.c.order_id == Stakeholder.id)
        return query
